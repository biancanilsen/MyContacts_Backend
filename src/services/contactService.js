...
const updateContact = async ({ id, nome, telefone, email }) => {
  ...
  return await Contact.update({ nome: nome, telefone: telefone, email: email, data_alteracao: Date.now() }, { where: { id: id } });
}
...
const deleteContact = async ({ id, userId }) => {
  ...
  return await Contact.update({ isActive: false }, { where: { id: id, userId: userId } });
}
